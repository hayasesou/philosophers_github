...
/*   Updated: 2023/11/04 18:50:08 by hfukushi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosopher.h"

t_return	print_philo_error(char *error_message, t_return return_val,
		const char *file, const char *func)
{
	printf("%s%s#[philo error]%s: %s   #file: %s   #func: %s\n",
		COLOR_RED, COLOR_BOLD, COLOR_RESET, error_message, file, func);
	return (return_val);
}
